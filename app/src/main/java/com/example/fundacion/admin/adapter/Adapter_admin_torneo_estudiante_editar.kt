...
class Adapter_admin_torneo_estudiante_editar
    (
    private val context: Context,
    private val estudiantes: List<lestudiantes>,
    private val refresh: FTorneo_estudiantes
): RecyclerView.Adapter<Adapter_admin_torneo_estudiante_editar.ViewHolder>()

{

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.aaa_list_torneoestudiantes_edit, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.cambiar.setOnClickListener {
            ...
            val postData = """
            {
                "IDTorneoEstudiantes": "${config.IDJuegoTorneo}",
                "IDEstu": "${dato.id}"
            }
        """.trimIndent()

            Fuel.post("${config.url}admin/torneo-estudiante-editar")
                .jsonBody(postData)
                .responseString{ _, _, result ->
                    result.fold(
                        success = { d ->

                            Toasty.success(context, "Estudiante Cambiado", Toasty.LENGTH_SHORT).show()
                            refresh.agregarEdit()
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var cambiar : Button = itemView.findViewById(R.id.cambiar)
